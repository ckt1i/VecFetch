...
ClusterProber::ClusterProber(const ConANN& conann, Dim dim, uint8_t code_bits, uint8_t total_bits)
    : ClusterProber(conann, dim, code_bits, total_bits,
                    total_bits > 1 ? code_bits : 0) {}
    ...
    for (uint32_t b = 0; b < pc.num_fastscan_blocks; ++b) {
        ...
        if (stage2_candidate_count > 0) {
            ...
            for (uint32_t bi = 0; bi < 4; ++bi) {
                ...
                uint32_t stage2_scatter_mask = block.lane_mask;
                ...
                double kernel_elapsed_ms = 0.0;
                    if (pc.exrabitq_storage_version >= 11) {
                        ...
                        const uint8_t layout_magnitude_bits =
                            block_view.loaded_magnitude_bits != 0
                                ? block_view.loaded_magnitude_bits
                                : block_view.magnitude_bits;
                        const uint8_t kernel_active_ex_bits =
                            std::min<uint8_t>(active_ex_bits_,
                                              layout_magnitude_bits);
                    ...
                    if (pc.uses_official_1_plus_n()) {
                        ...
                        auto record_stage2_kernel_call = [&](uint32_t mask,
                                                             uint8_t active_bits) {
                            const uint32_t lanes =
                                static_cast<uint32_t>(__builtin_popcount(mask));
                            stats.stage2_masked_kernel_calls += 1;
                            stats.stage2_lanes_requested += lanes;
                            stats.stage2_lanes_total_valid += block_view.valid_count;
                            stats.stage2_lanes_skipped += block_view.valid_count - lanes;
                            stats.stage2_active_ex_bits_sum +=
                                static_cast<uint64_t>(lanes) * active_bits;
                            stats.stage2_stored_ex_bits_sum +=
                                static_cast<uint64_t>(lanes) * block_view.magnitude_bits;
                        };
                        record_stage2_kernel_call(requested_mask,
                                                  kernel_active_ex_bits);
                            if (block_direct) {
                                if (block_view.exdata_layout ==
                                ...
                                simd::IPOfficialRaBitQBatchCompactDirect3Masked(
                                    ...
                                    pc.exrabitq_dim_block, ip_raw_batch, kernel_timing_ptr);
		                                ...
		                                simd::IPOfficialRaBitQBatchCompactDirectBitplanesStridedMasked(
		                                    rotated_query, kernel_abs_blocks, layout_magnitude_bits,
		                                ...
		                                simd::IPOfficialRaBitQBatchCompactTileLaneBitMajorMasked(
		                                    rotated_query, kernel_abs_blocks, layout_magnitude_bits,
		                                    requested_mask, block_view.valid_count, dim_,
		                                    pc.exrabitq_dim_block, ip_raw_batch, kernel_timing_ptr,
		                                    kernel_active_ex_bits);
                ...
                uint32_t chunk_idx = 0;
                for (uint32_t lane = 0; lane < kStage2BatchSize; ++lane) {
                    if ((stage2_scatter_mask & (1u << lane)) == 0)
                        continue;
                    const Stage2LaneEntry& entry = block.lanes[lane];
                    if (pc.uses_official_1_plus_n()) {
                ...
                if (!pc.uses_official_1_plus_n() && enable_stage2_scatter_batch_classify &&
                    ...
                    const simd::Stage2ClassifyMasks classify_masks = simd::Stage2ClassifyBatch(
                        lane_ip_raw, lane_xipnorm, lane_norm_oc, lane_margin_s1,
                        stage2_scatter_mask,
