...
OUT_ROOT=${OUT_ROOT:-$ROOT/test/recordgate_cold_cache_layout_pilot_20260718}
...
CPU=${CPU:-8}
PHASE=${PHASE:-all}
CACHE_MODE=drop-record-files-before-queries
CONFIGS=(adaptive_full c2s_no_combine)
...
layout_for() {
  case "$1" in
    adaptive_full) echo adaptive ;;
    c2s_no_combine) echo split ;;
...
validate_inputs() {
  ...
  [[ -s "$INPUT_ROOT/query_embeddings.npy" ]] || die "missing queries"
  ...
  jq -e --arg dataset "$DATASET_DIR" --arg index "$ADAPTIVE_STORE" \
    --argjson nprobe "$NPROBE" --argjson topk "$TOPK" '
      .schema_version == 2 and .dataset_dir == $dataset and
      .index_dir == $index and .method == "pcve" and
...
validate_result() {
  local result=$1 config=$2 layout
  layout=$(layout_for "$config")
  jq -e --arg layout "$layout" --arg split "$SPLIT_STORE" \
    --arg cache "$CACHE_MODE" --argjson topk "$TOPK" \
    --argjson nprobe "$NPROBE" '
      ...
      $m.dynamic_safeout_enabled == true and
      $m.dynamic_safein_mode == "static" and
      $m.materialization_mode == "late" and
      $m.safein_as_vec_only == true and
      $m.payload_cache_mode == $cache and
      $m.execution_mode == "overlap" and
      $m.vec_span_coalescing_enabled == true and
      $m.vec_span_planner_mode == "GV" and
      $m.vec_span_alpha_num == 3 and $m.vec_span_alpha_den == 2 and
      $m.vec_span_safein_rho_num == 0 and $m.vec_span_safein_rho_den == 1 and
      $m.vec_span_payload_reuse_enabled == true and
      $m.vec_span_payload_compact_enabled == false and
      $m.vec_span_safein_tail_count == 0 and
      (($layout == "adaptive" and $m.record_layout == "inline_hot_record_store" and
        $m.separate_store_dir == "") or
       ($layout == "split" and $m.record_layout == "no_combine_flatstor" and
        $m.separate_store_dir == $split and
...
run_one() {
  local config=$1 label=$2 queries=$3 output=$4 layout log
  local -a layout_args
  layout=$(layout_for "$config")
  ...
  echo "[run] $config $label queries=$queries cache=$CACHE_MODE"
...
run_smoke() {
  ...
  for config in "${CONFIGS[@]}"; do
    run_one "$config" smoke "$SMOKE_QUERIES" "$OUT_ROOT/smoke/$DATASET/$config"
  done
}

run_formal() {
  local rep config order
  for rep in $(seq 1 "$REPEATS"); do
    if (( rep % 2 == 1 )); then
      order="adaptive_full c2s_no_combine"
    else
      order="c2s_no_combine adaptive_full"
    fi
...
write_summary() {
  local summary=$OUT_ROOT/formal_summary.tsv config rep result
  printf '%s\n' \
    $'dataset\tconfig\trep\tqueries\trecall_at_10\tavg_ms\tp95_ms\tp99_ms\tqps\treranked\tvec_requests\tvec_bytes\tspan_requests\tspan_candidates\tspan_bytes\tplanned_physical_bytes\tplanned_vector_bytes\trealized_amplification\tpayload_requests\tpayload_bytes\treuse_hits\treuse_bytes\trequests_avoided\ttotal_read_bytes\ttotal_io_submitted\tprobe_ms\tfetch_missing_ms\tio_wait_ms\tplanner_ms\tsort_ms\tplanner_fallbacks\tpeak_rss_kib' \
    >"$summary"
  for config in "${CONFIGS[@]}"; do
    for rep in $(seq 1 "$REPEATS"); do
      ...
      jq -r --arg dataset "$DATASET" --arg config "$config" --arg rep "$rep" '
        ...
        [$dataset,$config,$rep,$m.num_queries,$m.recall_at_10,
         $m.avg_query_time_ms,$m.p95_ms,$m.p99_ms,(1000/$m.avg_query_time_ms),
         $p.avg_candidates_reranked,$p.avg_vec_only_read_requests,
         $p.avg_vec_only_read_bytes,$p.avg_vec_span_read_requests,
         $p.avg_vec_span_candidates,$p.avg_vec_span_read_bytes,
         $p.avg_vec_span_planned_physical_bytes,$p.avg_vec_span_planned_vector_bytes,
         $amp,$p.avg_payload_read_requests,$p.avg_payload_read_bytes,
         $p.avg_vec_span_payload_reuse_hits,$p.avg_vec_span_payload_reuse_bytes,
         $p.avg_vec_span_payload_requests_avoided,$p.avg_total_read_bytes,
         $p.avg_total_io_submitted,$p.avg_probe_ms,$p.avg_fetch_missing_ms,
         $p.avg_io_wait_ms,$p.avg_vec_span_planner_ms,$p.avg_vec_span_sort_ms,
         $p.avg_vec_span_planner_fallbacks,.rss_profile.peak_during_queries_rss_kib] | @tsv
      ' "$result" >>"$summary"
    done
  done
