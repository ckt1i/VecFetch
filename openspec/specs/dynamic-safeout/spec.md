...
- [ ] SafeIn 使用 SafeIn 专用 `safein_d_k`（可用时），不再使用 `dynamic_d_k`
- [ ] est_heap_ 未满时 SafeOut 不由静态 `d_k` 触发（dynamic_d_k = infinity）
- [ ] 单元测试验证 ClassifyAdaptive 的三分类正确性
- [ ] 旧索引在无 SafeIn 专用 metadata 时 SafeIn 回退到 legacy `d_k_`
