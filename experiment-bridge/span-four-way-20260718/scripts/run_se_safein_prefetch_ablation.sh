...
OUT_ROOT=${OUT_ROOT:-$ROOT/test/recordgate_span_se_safein_prefetch_ablation_20260718}
...
validate_result() {
  ...
  jq -e --argjson expected_vec_only "$expected_vec_only" \
      ...
      --argjson rho_num "$(rho_num)" --argjson rho_den "$(rho_den)" '
    .metrics as $c | .pipeline_stats as $p |
    $c.vec_span_planner_mode == "SE" and
    ...
    (if $expected_vec_only then $p.avg_all_read_requests == 0 else true end)
  ' "$result_file" >/dev/null || {
    echo "invalid SafeIn-prefetch ablation contract: $result_file" >&2
    return 3
  }
}
...
run_one() {
  ...
  VDB_RESIDENT_HUGEPAGE=0 taskset -c "$CPU" "$BINARY" \
    ...
    --vec-span-planner-mode SE --vec-span-alpha=3/2 \
    --vec-span-safein-rho="$RHO" --vec-span-payload-reuse 1 \
...
for rep in $(seq 1 "$REPS"); do
  for dataset in $DATASETS; do
    ...
    warm="$OUT_ROOT/rep$rep/warmup/q$WARMUP_QUERIES/$dataset/SE"
    [[ -s "$warm/results.json" ]] || \
...
echo "SafeIn-prefetch ablation results: $OUT_ROOT"
